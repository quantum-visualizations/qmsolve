...
class VisualizationIdenticalParticles1D(Visualization):
    ...
    def plot_eigenstate(self, k):
        ...
        im = ax1.imshow((eigenstates_array[k]), aspect = "auto", extent = [-L, L, -L, L], cmap ='seismic',  interpolation = 'bilinear',  origin='lower')
        x = np.linspace(-L, L, self.eigenstates.N)

        prob_density = np.sum(  (eigenstates_array[k])**2  , axis = 1)

        ax3.plot(x,  prob_density)
    ...
    def slider_plot(self):
        ...
        grid = plt.GridSpec(2, 2, width_ratios=[4.5, 1], height_ratios=[2.5, 1] , hspace=0.3, wspace=0.4)
        ax1 = fig.add_subplot(grid[0:1, 0:1])
        ax3 = fig.add_subplot(grid[1:2, 0:1], sharex=ax1) # probability density of finding any particle at x 
        ax2 = fig.add_subplot(grid[0:2, 1:2])
        ...
        L =  self.eigenstates.extent/2
        eigenstate_plot = ax1.imshow((eigenstates_array[0]), aspect = "auto", extent = [-L, L, -L, L], cmap ='seismic',  interpolation = 'bilinear',  origin='lower')
        x = np.linspace(-L, L, self.eigenstates.N)

        prob_density = np.sum(  (eigenstates_array[0])**2  , axis = 1)
        prob_plot = ax3.plot(x,  prob_density)
        ...
        def update(state):
            ...
            prob_density = np.sum(  (eigenstates_array[state])**2  , axis = 1)
            prob_plot[0].set_ydata(prob_density)
    ...
    def animate(self):
        ...
        def func_animation(*arg):
            ...
            if (animation_data['n'] % 1.0) > 0.5:
                ...
                eigenstate_plot.set_data(eigenstate_combination)
                prob_density = np.sum(  eigenstate_combination**2  , axis = 1)

                prob_plot.set_ydata(prob_density)
                ...
                prob_plot.set_ydata(np.sum(  (eigenstates_array[state])**2  , axis = 1))
            return eigenstate_plot, line,prob_plot

        a = animation.FuncAnimation(fig, func_animation,
                                    blit=True, interval=1.0,frames = 300)
        #plt.show()
